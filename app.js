...
if(message=='/spam'){
    ...
    async function retardo() {
        for (i=0;i<masivo.length;i++) {
            ...
            if(i % 15 === 0){console.log("********  VAN 15, HACEMOS PAUSA DE 10 SEGUNDOS ********"); await sleep(10000);} 
            console.log(`=============   Mandamos el mensaje ${i}   ==============`);
            var elTextoDelMensaje = "Este es un mensaje de prueba"
            await sleep(500);
            console.log(i, masivo[i].numero+"@c.us", elTextoDelMensaje);
            client.sendMessage(masivo[i].numero+"@c.us", remplazos(elTextoDelMensaje, client));
            console.log(`Esperamos ${rnd} segundos...`);
...
if(message=='/spam'){
    ...
    async function retardo() {
        for (q=0;q<masivo.length;q++) {
            console.log(masivo[q].numero+"@c.us");
            var rnd = getRandomInt(1,7); // Random entre 1 y 6 segundos.
            if(q % 15 === 0){console.log("********  VAN 15, HACEMOS PAUSA DE 10 SEGUNDOS ********"); await sleep(10000);} //
            console.log(`=============   Mandamos el mensaje ${q}   ==============`);
            var elTextoDelMensaje = "¡Hola! Soy lariBot! 🤖🧞‍♂️👀\n*Este es un mensaje de prueba.*✔🐱‍🚀 \n Esto permitirá que toda información, actualización y/o información sea enviado a cada uno de ustedes con la finalidad de un mejor y óptimo desempeño  Si leíste este mensaje, por favor responde \n *OK*"
            await sleep(500);
            console.log(q, masivo.length, masivo[q].numero+"@c.us", elTextoDelMensaje)
            if(!isValidNumber(masivo[q].numero+"@c.us")){ console.log('Numero INVALIDO')}
            sendMessage(client, masivo[q].numero+"@c.us", remplazos(elTextoDelMensaje, client));
            console.log(`Esperamos ${rnd} segundos...`);
            await sleep(rnd*1000);
        }
